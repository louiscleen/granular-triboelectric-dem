...
double patch_angle_from_index(int patch_index, int n) {
    return ((patch_index + 1) * (2.0 * M_PI / (double)n) + patch_index * (2.0 * M_PI / (double)n)) /
           2.;
}

bool compute_disk_disk_contact(Disk &i_disk, Disk *j_disk_ptr, double i_deltan,
                               Eigen::Vector3d &i_n, double i_kn, double i_e, double i_mu,
                               bool toggle, double q, double qmax) {
    bool ret = false;
    ...
    i_n.normalize();
    Eigen::Vector3d v =
        b->v() + b->radius() * b->w().cross(i_n) - (a->v() - a->radius() * a->w().cross(i_n));
    ...
    if (Fn > 0.) {
        ...
        // On a un transfert de charge si un patch donneur touche un patch accepteur (et
        // réciproquement)
        ...
        if (a->getContactChargeTransfer(a_patch_index, b->index(), b_patch_index) == false &&
            b->getContactChargeTransfer(b_patch_index, a->index(), a_patch_index) ==
                false) // Si pas de transfert déjà effectué dans ce contact
        {
...
void compute_disk_circular_piston_contact(Disk &i_disk, Disk *j_disk_ptr, double i_deltan,
                                          Eigen::Vector3d &i_n, double i_kn, double i_e,
                                          double i_mu, int grounded_walls) {
    Disk *a = &i_disk;
    Disk *b = j_disk_ptr;
    ...
    if (grounded_walls == 1) {
        double angle =
            std::atan2(i_n.y(), i_n.x()) + M_PI;    // Direction du point de contact côté disque
...
void compute_disk_wall_contact(Disk &i_disk, double i_deltan, Eigen::Vector3d &i_n, double i_kn,
                               double i_e, double i_mu, int grounded_walls) {
    // contact base
    i_n.normalize();
...
void compute_screened_coulomb_interaction(Disk &a, Disk &b, double ke, double inv_lambda,
                                          double r_soft, double r_cut2,
                                          std::vector<double> &patch_angle_cache, int n_patch,
                                          double fast_r_cut2) {
    const double rad_a = a.radius();
    const double rad_b = b.radius();
    const Eigen::Vector3d n = b.r() - a.r();
    if (n.squaredNorm() > fast_r_cut2)
        return;
...
double Ep_compute_screened_coulomb_interaction(Disk &a, Disk &b, double ke, double inv_lambda,
                                               double r_soft, double r_cut2,
                                               std::vector<double> &patch_angle_cache, int n_patch,
                                               double fast_r_cut2) {
    double Ep = 0.0;
