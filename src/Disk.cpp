...
Disk::Disk(int i_index, int i_patch, int i_sat, double i_qmax, vector<bool> i_patch_state,
           double i_radius, double i_mass, double i_x, double i_y, double i_vx, double i_vy) {
    m_index = i_index;
    m_linkedDisk = nullptr;
    m_linkedCell = nullptr;
    m_radius = i_radius;
...
void Disk::add_force(const Eigen::Vector3d &i_F) { m_F += i_F; }

void Disk::add_momentum(const Eigen::Vector3d &i_M) { m_M += i_M; }

void Disk::add_gravity_force(const Eigen::Vector3d &i_g) { m_F += m_mass * i_g; }
...
void Disk::set_linked_disk(Disk *i_linkedDisk) { m_linkedDisk = i_linkedDisk; }

void Disk::set_linked_cell(Cell *i_linkedCell) { m_linkedCell = i_linkedCell; }
...
void Disk::print(int i_num) {
    ...
    myfile.precision(10);
    myfile << m_index << "\t" << m_r.x() << "\t" << m_r.y() << "\t" << m_v.x() << "\t" << m_v.y()
           << "\t" << m_theta << "\t" << m_radius << endl;
    myfile.close();
}

Disk *Disk::linked_disk() { return m_linkedDisk; }

Cell *Disk::linked_cell() { return m_linkedCell; }

int Disk::index() { return m_index; }

double Disk::radius() { return m_radius; }

double Disk::mass() { return m_mass; }

Eigen::Vector3d Disk::r() { return m_r; }

Eigen::Vector3d Disk::v() { return m_v; }

double Disk::theta() { return m_theta; }

Eigen::Vector3d Disk::w() { return m_w; }
...
void Disk::setCaged(bool state) { caged = state; }

bool Disk::isCaged() { return caged; }

int Disk::patch_count() { return m_patch; }

std::vector<bool> Disk::getPatchStates() { return m_patch_state; }

void Disk::set_charge(int patch_index, double charge) { m_patch_charges[patch_index] = charge; }

void Disk::add_charge(int patch_index, double charge) { m_patch_charges[patch_index] += charge; }

void Disk::multiply_charge(int patch_index, double factor) {
    m_patch_charges[patch_index] *= factor;
}

void Disk::reset_patch_charges() { std::fill(m_patch_charges.begin(), m_patch_charges.end(), 0.0); }

void Disk::reset_patch_charge(int patch_index) { m_patch_charges[patch_index] = 0.0; }

std::vector<double> Disk::getPatchCharges() { return m_patch_charges; }

double Disk::getPatchCharge(int patch_index) { return m_patch_charges[patch_index]; }

int Disk::get_sat() { return m_sat; }
...
void Disk::addContact(int myPatchIndex, int neighborIndex, int neighborPatchIndex, bool state) {
    ...
    for (auto &contact : contacts) {
        if (contact.patchIndex == myPatchIndex && contact.neighborIndex == neighborIndex &&
            contact.neighborPatchIndex == neighborPatchIndex) {
...
std::vector<Disk::ContactInfo> &Disk::getContacts() { return contacts; }
...
void Disk::setContactState(int myPatchIndex, int neighborIndex, int neighborPatchIndex,
                           bool state) {
    for (auto &contact : contacts) {
        if (contact.patchIndex == myPatchIndex && contact.neighborIndex == neighborIndex &&
            contact.neighborPatchIndex == neighborPatchIndex) {
...
void Disk::setContactChargeTransfer(int myPatchIndex, int neighborIndex, int neighborPatchIndex,
                                    bool hasTransferredCharge) {
    for (auto &contact : contacts) {
        if (contact.patchIndex == myPatchIndex && contact.neighborIndex == neighborIndex &&
            contact.neighborPatchIndex == neighborPatchIndex) {
            contact.hasTransferredCharge =
                hasTransferredCharge; // Met à jour l'état du transfert de charge
            return;
        }
    }
}

double Disk::inertia() { return m_inertia; }