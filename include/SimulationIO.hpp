...
public:
    ...
    SimulationWriter(const std::string& filename, const config::Config& cfg): save_mode_(cfg.output.save_mode)
    {
        if (save_mode_ == 2)
        {
            out.open(filename);
            ...
            if (!out) throw std::runtime_error("Impossible d'ouvrir le fichier");
            ...
            params.shape = cfg.boundaries.oscillation.shape; // 's' = straight, 'i' = inclined, 'c' = circular
...
public:
    ...
    SimulationReader(const std::string& filename, int save_mode = 1) : save_mode_(save_mode) {
        ...
        if (save_mode_ == 2) {
            in.open(filename);
            if (!in) throw std::runtime_error("Impossible d'ouvrir le fichier");
